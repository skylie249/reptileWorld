document.addEventListener('DOMContentLoaded', () => {
    ...
    const loadPosts = () => {
        ...
        recentPosts.forEach(post => {
            ...
            card.innerHTML = `
                <button class="feed-delete" onclick="deletePost(${post.id})" title="${t('mypage_del_title') || '글 삭제'}"><i data-lucide="trash-2" style="width: 16px; height: 16px;"></i></button>
        ...
        for (let i = 0; i < placeholdersNeeded; i++) {
            ...
            dummyCard.innerHTML = `
                <div style="width: 100%; height: 220px; background: rgba(255,255,255,0.05); border-radius: 8px; margin-bottom: 0.5rem; display: flex; align-items: center; justify-content: center; flex-direction: column; color: var(--text-muted);">
                    ...
                    <span style="font-size: 0.8rem;">${t('mypage_dummy_img')}</span>
                </div>
                <div class="feed-date" style="font-size: 0.8rem; color: var(--text-muted); margin-bottom: 0.5rem; margin-top: 0.5rem;">${t('mypage_dummy_date')}</div>
                <p class="feed-preview" style="color: var(--text-muted);">${t('mypage_dummy_desc')}</p>
            `;
    ...
    window.deletePost = (id) => {
        if(confirm(t('mypage_alert_delete') || '이 다이어리 기록을 정말 삭제하시겠습니까?')) {
    ...
    if (submitBtn) {
        submitBtn.addEventListener('click', () => {
            ...
            if (!text && !selectedImageBase64) {
                alert(t('mypage_alert_empty') || '내용을 작성하거나 파충류 사진을 등록해 주세요.');
                return;
            }
            ...
            try {
                ...
                alert(t('mypage_alert_full') || '로컬 저장소 용량이 초과되었습니다. 기존 데이터를 일부 삭제해 주세요.');
                return;
            }
    ...
    if (feedContainer) {
        loadPosts();
        window.addEventListener('languageChanged', loadPosts);
    }
});
